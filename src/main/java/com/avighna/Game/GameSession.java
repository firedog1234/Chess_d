...
public class GameSession implements Runnable {
  ...
  public boolean validateAndMove(JsonHolder holder) {
    ...
    Piece.Position from = new Piece.Position(sourceRow, sourceColumn);
    Piece.Position to = new Piece.Position(targetRow, targetColumn);

    boolean isValid = board.getPiece(from).isValidMove(from, to, board);
  ...
  public void run() {
    ...
    while (true) {
      if (this.isBlacksMove()) {
        ...
        try {
          GameSession.getServer()
              .sendBlackMove(
                  store.getStartingSquare(), store.getEndingSquare(), this.getConnection());
        } catch (JsonProcessingException e) {
          throw new RuntimeException(e);
        }
        board.movePiece(
            8 - store.getStartingSquare().first,
            store.getStartingSquare().second,
            8 - store.getEndingSquare().first,
            store.getEndingSquare().second);
      }
